...
public class FeePerKbSupport {
    ...
    Coin genesisFeePerKb;
    AddressBasedAuthorizer feePerKbChangeAuthorizer;
    BridgeStorageProvider provider;

    public FeePerKbSupport(
        Coin genesisFeePerKb,
        AddressBasedAuthorizer feePerKbChangeAuthorizer,
        BridgeStorageProvider provider
    ) {
    ...
    public Integer voteFeePerKbChange(Transaction tx, Coin feePerKb) {
        if (!feePerKbChangeAuthorizer.isAuthorized(tx, signatureCache)) {
